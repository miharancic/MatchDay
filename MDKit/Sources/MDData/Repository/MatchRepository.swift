...
public actor MatchRepository: MatchRepositoryType {
    ...
    private var competitions: [CompetitionEntity] = []
    ...
    public func loadStored() async throws {
        sports = try await matchStore.loadSports()
        competitions = try await matchStore.loadCompetitions()
        matches = try await matchStore.loadMatches()
    }
    
    public func getAllSports() -> [SportEntity] {
        return sports
    }
    ...
    public func getAllCompetitions() -> [CompetitionEntity] {
        return competitions
    }
    ...
    public func getAllMatches() -> [MatchEntity] {
        return matches
    }
